...
**Collection join을 페이징과 함께 조회하려면..?**

1. 먼저 XToOne 관계는 모두 페치조인을 걸어준다. -> 페이징 쿼리에는 영향을 주지 않음
2. 컬렉션은 지연로딩으로 조회한다.
3. 지연로딩 최적화를 위해 `hibernate.default_batch_fetch_size`, `@BatchSize` 를 설정한다.
4. 설정한 size만큼 IN 쿼리를 통해 조회한다.

Default_batch_fetch_size는 100~1000사이를 선택하는걸 권장한다. 부하를 견딜 수 있는 수준을 설정하도록 하자.

최종적으로 데이터를 가져와야하는 데이터갯수는 동일하다. 
